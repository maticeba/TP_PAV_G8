    ...
    public partial class Frm_Detalle_SPrepago : Form
    {
        ...
        private void CargarDatos(DataTable tabla)
        {
            ...
            for (int i = 0; i < tabla.Rows.Count; i++)
            {
                ...
                grid_Serv_Prepagos.Rows[i].Cells[4].Value = cuota;
                ...
                grid_Serv_Prepagos.Rows[i].Cells[5].Value = descuento;
                grid_Serv_Prepagos.Rows[i].Cells[6].Value = tabla.Rows[i]["costo"].ToString();
                if (descuento != "")

                {
                    grid_Serv_Prepagos.Rows[i].Cells[6].Value = ((float.Parse(costo) - (float.Parse(costo) * float.Parse(descuento) / 100))).ToString();

                }
                else
                {
                    grid_Serv_Prepagos.Rows[i].Cells[6].Value = costo;

                }
                precio += float.Parse(grid_Serv_Prepagos.Rows[i].Cells[6].Value.ToString());
            }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("¿Desea cargar el detalle de servicio?", "Importante", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                ...
                for (int i = 0; i < grid_Serv_Prepagos.Rows.Count; i++)
                {
                    ...
                    Detalle.Pp_descuento = grid_Serv_Prepagos.Rows[i].Cells[5].Value.ToString();
